...
const Question = ({ index, handleNext, handlePrev, answer, setAnswer, setScore, setDisplay }) => {
  ...
  return (
    <div className="h-full relative p-[54px] font-alfa flex flex-col gap-[54px]">
      <>
        <div className="flex text-white text-[28px] justify-center text-center">
          {`Question ${index + 1} : ${questions[index].question}`}
        </div>
        <div className="grid grid-cols-2 gap-x-[142px] gap-y-[42px] mx-[20px] mt-[-20px]">
          {Object.entries(questions[index].options).map(([key, val]) => {
            return (
              <div
                onClick={() => handleAns(key, index)}
                key={key}
                className={` h-[111px] rounded-[20px] text-[18px] flex items-center text-white p-4 cursor-pointer hover:bg-pinkCerise hover:border-4 hover:border-white ${
                  answer[index+1] === key
                    ? "bg-pinkCerise border-4 border-white"
                    : "bg-[#8e44ad]"
                }`}
              >
                {`${key}, ${val}`}
      ...
      <div className="absolute bottom-[10px] right-[54px] flex justify-end mx-[20px] gap-5">
        {index !== 0 && (
          <svg
            onClick={handlePrev}
            ...
            height="63"
            viewBox="0 0 72 73"
            className="bg-pinkCerise rounded-full border-solid border-[6px] border-lightPink hover:border-white hover:scale-110 hover:shadow-2xl hover:fill-white "
            fill="#ffcccc"
          >
        ...
        {index + 1 === questions.length && (
          <div onClick={handleSubmit} className="text-[30px] bg-pinkCerise flex items-center p-1 text-lightPink border-[6px] border-lightPink hover:scale-110 hover:text-white hover:border-white rounded-lg">
            Finish
          </div>
        )}
      </div>
